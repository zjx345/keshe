 ...
 *
 */
public class StudentUpdatePasswordWindow extends JInternalFrame {

	private JPasswordField passwordField;
	private JPasswordField passwordField_1;
	private JPasswordField passwordField_2;
	private static StudentUpdatePasswordWindow updatePasswordWindow=new StudentUpdatePasswordWindow();
	private Student stu;
	/**
	 * 该私有方法为无参构造方法，用来初始化该窗口
	 */
	private StudentUpdatePasswordWindow() {
		setClosable(true);
		setIconifiable(true);
		setMaximizable(true);
		setTitle("\u4FEE\u6539\u5BC6\u7801");
		...
		submitButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if(stu.getPassword().equals(passwordField.getText())) {
					if(passwordField_1.getText().equals(passwordField_2.getText())) {
						if(ServiceFactory.getIStudentServiceInstance().updatePassword(stu, passwordField_1.getText())) {
	 ...
	 * @return 返回该窗口对象
	 */
	public static StudentUpdatePasswordWindow getInstance() {
